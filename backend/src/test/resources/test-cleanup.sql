...
-- post_embeddings → posts → users → vector_configs 순서로 삭제
...
-- 4. VectorConfig 테이블 정리 (외래 키 없음)
DELETE FROM vector_configs;

-- 시퀀스 리셋 (AUTO_INCREMENT ID 초기화)
ALTER SEQUENCE users_id_seq RESTART WITH 1;
ALTER SEQUENCE posts_id_seq RESTART WITH 1;
ALTER SEQUENCE post_embeddings_id_seq RESTART WITH 1;
ALTER SEQUENCE vector_configs_id_seq RESTART WITH 1;
